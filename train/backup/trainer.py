...
from LaplacianGan.misc.config import cfg
from LaplacianGan.misc.utils import mkdir_p

TINY = 1e-8

# reduce_mean normalize also the dimension of the embeddings
def KL_loss(mu, log_sigma):
    loss = -log_sigma + .5 * (-1 + torch.exp(2. * log_sigma) + torch.square(mu))
...
class CondGANTrainer(object):
    ...
    def sample_encoded_context(self, embeddings):
        ...
        if cfg.TRAIN.COND_AUGMENTATION:
            ...
            epsilon = torch.randn(mean.size()) #tf.truncated_normal(tf.shape(mean))
            stddev  = torch.exp(c_mean_logsigma[1])
    ...
    def forward(self, embeddings):
        ...
        c, kl_loss = self.sample_encoded_context(embeddings)
        #z = tf.random_normal([self.batch_size, cfg.Z_DIM])
        
        fake_images = self.model.generator(tf.concat(1, c))
    ...
    def compute_losses(self, images, wrong_images, fake_images, embeddings):
        ...
        ones_target  =  Variable(real_logit.new(real_logit.size())._fill(1.0))
        zeros_target =  Variable(real_logit.new(real_logit.size())._fill(0.0))
        real_d_loss =\
