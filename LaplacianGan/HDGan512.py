...
def train_gans_super(dataset, model_root, mode_name, netG, netD, args):
    print('>> using hd super gan trainer')
    # helper function
    ...
    if args.reuse_weights :
        ...
        try:
            G_weightspath = os.path.join(model_folder, 'G_epoch_256_init.pth')
            weights_dict = torch.load(G_weightspath, map_location=lambda storage, loc: storage)
            print ('load pretrained 256 generator from '+G_weightspath)
            load_partial_state_dict(netG.generator_256, weights_dict)
            for p in netG.generator_256.parameters(): p.requires_grad = False
            weights_dict = None
    ...
    fixed_images, _, fixed_embeddings, _, _ = test_sampler(args.batch_size, 1, super_resolution=True)
    ...
    for epoch in range(start_epoch, tot_epoch):
        ...
        for idx_test in range(num_test_forward + 1):
            ...
            if idx_test == 0:
                ...
                test_images, _, test_embeddings, _, _ = test_sampler(args.batch_size, 1, super_resolution=True)
