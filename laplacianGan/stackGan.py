...
def compute_g_loss(fake_pair_logit, fake_img_logit, wgan=False):
    if wgan:
        ...
        ones_target  =  Variable(fake_pair_logit.data.new(fake_pair_logit.size()).
                                fill_(1.0), requires_grad=False)
        #zeros_target =  Variable(real_logit.data.new(real_logit.size()).fill_(0.0), requires_grad=False)
        generator_loss = F.binary_cross_entropy_with_logits(fake_pair_logit, ones_target) \
                       +  F.binary_cross_entropy_with_logits(fake_img_logit, ones_target)
        #generator_loss = torch.mean(-F.logsigmoid(fake_pair_logit) - F.logsigmoid(fake_img_logit)) 
            ...
            for key, _ in fake_images.items():
                ...
                if use_content_loss:
                    if batch_count > 1000:
                        ...
                        #l1 = torch.mean(torch.abs(fake_img_code - real_img_code))* 4
                        conten_loss = GaussianLogDensity(real_img_code, fake_img_code)
                        #l1 = torch.mean(torch.abs(this_fake - this_img))* 4
